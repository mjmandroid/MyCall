...
public class SuspensionBrandDecoration extends RecyclerView.ItemDecoration {
    ...
    public SuspensionBrandDecoration(Context context, List<? extends ISuspensionInterface> datas) {
        this.mDatas = datas;
    ...
    public SuspensionBrandDecoration setmTitleHeight(int mTitleHeight) {
        this.mTitleHeight = mTitleHeight;
        return this;
    }

    public SuspensionBrandDecoration setColorTitleBg(int colorTitleBg) {
        COLOR_TITLE_BG = colorTitleBg;
        return this;
    }

    public SuspensionBrandDecoration setColorTitleFont(int colorTitleFont) {
        COLOR_TITLE_FONT = colorTitleFont;
        return this;
    }

    public SuspensionBrandDecoration setTitleFontSize(int mTitleFontSize) {
        this.mPaint.setTextSize((float)mTitleFontSize);
        return this;
    }

    public SuspensionBrandDecoration setmDatas(List<? extends ISuspensionInterface> mDatas) {
        this.mDatas = mDatas;
        return this;
    }
    ...
    public SuspensionBrandDecoration setHeaderViewCount(int headerViewCount) {
    ...
    private void drawTitleArea(Canvas c, int left, int right, View child, RecyclerView.LayoutParams params, int position) {
        ...
        c.drawText(((ISuspensionInterface)this.mDatas.get(position)).getSuspensionTag(), (float)child.getPaddingLeft()+Utility.dip2px(mContext,15), (float)(child.getTop() - params.topMargin - (this.mTitleHeight / 2 - this.mBounds.height() / 2)), this.mPaint);
    }
    ...
    public void onDrawOver(Canvas c, RecyclerView parent, RecyclerView.State state) {
        ...
        if (this.mDatas != null && !this.mDatas.isEmpty() && pos <= this.mDatas.size() - 1 && pos >= 0 && ((ISuspensionInterface)this.mDatas.get(pos)).isShowSuspension()) {
            ...
            c.drawText(tag, (float)child.getPaddingLeft()+ Utility.dip2px(mContext,15), (float)(parent.getPaddingTop() + this.mTitleHeight - (this.mTitleHeight / 2 - this.mBounds.height() / 2)), this.mPaint);
            if (flag) {
                c.restore();
            }
